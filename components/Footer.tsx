...
export default function Footer() {
  return (
    <footer className="border-t border-line bg-[#120e0b]">
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 md:grid-cols-12 md:px-8">
        <div className="md:col-span-5">
          ...
          <p className="mt-5 max-w-sm text-sm leading-relaxed text-cream-dim">
            We carve functional art. Traditional mortise-and-tenon joinery using certified
            sustainable American hardwoods. Crafted individually, made for lifetimes.
          </p>
          <div className="mt-6 flex gap-3">
            {[Facebook, Instagram, Mail].map((Icon, i) => (
              <a
                key={i}
                href="#contact"
                className="grid h-9 w-9 place-items-center rounded-full border border-line-strong text-cream-dim hover:text-gold hover:border-gold transition-colors"
              >
                <Icon size={14} />
              </a>
            ))}
        ...
        <div className="md:col-span-2">
          ...
          <ul className="mt-5 space-y-2.5">
            {quick.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="text-sm text-cream-dim hover:text-gold transition-colors">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div className="md:col-span-5">
          ...
          <ul className="mt-5 space-y-3 text-sm text-cream-dim">
            ...
            <li className="flex items-center gap-2.5">
              <Mail size={14} className="text-gold" /> studio@woodcraftdesign.com
            </li>
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-5 text-[11px] uppercase tracking-[0.16em] text-cream-dim/70 md:flex-row md:items-center md:justify-between md:px-8">
          <p>© {new Date().getFullYear()} Wood Craft &amp; Design. All rights reserved.</p>
          <div className="flex gap-6">
            <Link href="/privacy">Privacy Policy</Link>
            <Link href="/terms">Terms of Service</Link>
            <Link href="/philosophy">Timber Sourcing</Link>
          </div>
        </div>
      </div>
    </footer>
